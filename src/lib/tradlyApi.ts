...
async function resolveCurrency(): Promise<string> {
  ...
  try {
    const res = await request<{ data: { currencies: TradlyTenantCurrency[] } }>(
  ...
  throw new Error('Unable to resolve currency from Tradly API for this tenant.');
}
...
export async function checkoutTradlyCart(
  ...
  const res = await withCurrencyRetry(async () => {
    const currency = await resolveCurrency();
    return request<{ data: { order: TradlyOrder } }>(
...
export async function getTradlyOrders(
  ...
  const res = await request<{ data: { orders: TradlyOrder[]; total_count?: number } }>(
    `/products/v1/orders?${qs.toString()}`,
    {},
    authKey,
  );
  return {
...
function mapCategoryNameToLocal(name: string): string {
  ...
  if (/food|meal|bakery|bread|restaurant|kitchen|snack|beverage/i.test(text)) return 'food';
  if (/service|repair|clean|maintenance|home.?service|booking|appointment/i.test(text)) return 'services';
  return 'grocery';
}
...
export function seedCategoryMap(categories: TradlyCategory[]): void {
  ...
  for (const c of categories) {
    ...
    LOCAL_CATEGORY_IDS[localCategory] = [...(LOCAL_CATEGORY_IDS[localCategory] ?? []), c.id];
  }
}

export function getTradlyCategoryIdsByLocalCategory(localCategory: string): number[] {
...
function inferCategoryFromText(text: string): string {
  ...
  if (/repair|service|clean|maintenance|booking|appointment|technician|maid/i.test(text)) return 'services';
  if (/food|meal|lunch|dinner|bakery|bread|rice|chicken|egg|coffee|tea/i.test(text)) return 'food';
  return 'grocery';
}

function resolveCategory(categoryIds: number[], title: string, description: string): string {
...
export function adaptListingToProduct(
  ...
  const category = resolveCategory(listing.category_id, listing.title, listing.description ?? '');
